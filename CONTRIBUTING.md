...
current Linux, Windows-backend, and macOS checks.
...
On a matching Mac, build one native worker with `macos-intel` or
`macos-arm64`. The build helper ad-hoc signs Apple release binaries.
...
- Avoid increasing memory use or concurrency without measurements on
