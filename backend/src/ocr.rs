        ...
        .filter_map(|paragraph| {
            let lines: Vec<_> = paragraph
                ...
                .filter_map(|line| {
                    ...
                    let geometry = line.geometry.as_ref()?;
                    let polygon = geometry_polygon(
                        geometry,
                        image_width,
                        chunk_height,
                        chunk_y,
                    ...
                    (!text.is_empty()).then_some((text, polygon))
                })
                .collect();
            ...
            // Lens normally supplies line grouping. Older or unusual responses
            // may only have paragraph-level geometry, in which case retaining
            // a selectable line is better than dropping the text entirely.
...
mod tests {
    ...
    use super::{
        DetectedGeometry, DetectedLine, DetectedParagraph, blocks_from_paragraphs,
        group_nearby_blocks, image_dimensions,
    };
    ...
    #[test]
    fn groups_exact_top_bubble_geometry_from_device_cache() {
        let blocks = vec![
            MokuroBlock::new(
                [782, 50, 802, 225],
                true,
                20,
                vec![polygon([[782, 50], [801, 50], [802, 225], [782, 225]])],
                vec!["右列テスト".to_owned()],
            ),
            MokuroBlock::new(
                [708, 45, 767, 238],
                ...
                25,
                vec![
                    polygon([[742, 49], [766, 49], [767, 238], [742, 238]]),
                    polygon([[708, 45], [729, 45], [729, 96], [708, 96]]),
                ],
                vec!["中央列テスト".to_owned(), "終端".to_owned()],
            ),
        ];
        ...
        assert_eq!(grouped.len(), 1);
        assert_eq!(grouped[0].lines, ["右列テスト", "中央列テスト", "終端"]);
        assert_eq!(grouped[0].box_, [708, 45, 802, 238]);
    }

    #[test]
    fn groups_exact_bottom_right_columns_from_device_cache() {
        let blocks = vec![
            MokuroBlock::new(
                [1167, 1317, 1300, 1725],
                true,
                22,
                vec![
                    polygon([[1278, 1501], [1300, 1501], [1300, 1578], [1278, 1578]]),
                    polygon([[1281, 1626], [1295, 1626], [1295, 1642], [1281, 1642]]),
                    polygon([[1241, 1322], [1278, 1322], [1281, 1725], [1244, 1725]]),
                    polygon([[1186, 1322], [1225, 1322], [1226, 1613], [1187, 1613]]),
                    polygon([[1171, 1317], [1190, 1317], [1190, 1359], [1171, 1359]]),
                    polygon([[1167, 1549], [1186, 1549], [1186, 1617], [1167, 1617]]),
                ],
                vec![
                    "よみいち".to_owned(),
                    "よ".to_owned(),
        ...
        assert_eq!(grouped[0].box_, [1132, 1317, 1300, 1725]);
    }
