...
if [[ "$target_triple" == *-apple-darwin ]]; then
    if [[ "$(uname -s)" != "Darwin" ]]; then
        printf 'error: Apple workers must be built and signed on macOS\n' >&2
        exit 1
    fi
    if ! command -v codesign >/dev/null 2>&1; then
        printf 'error: codesign is required for macOS release workers\n' >&2
        exit 1
    fi
    # Ad-hoc signing gives both Intel and Apple Silicon workers a stable code
    # signature. It does not notarize the plugin or bypass Gatekeeper.
