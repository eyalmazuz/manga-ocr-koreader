...
local function declareFFI()
    if not pcall(ffi.typeof, "posix_spawn_file_actions_t") then
        local fallback_type
        if ffi.os == "OSX" then
            -- Darwin exposes this as an opaque pointer in <spawn.h>.
            fallback_type = "typedef void *posix_spawn_file_actions_t;"
        else
            -- Linux headers expose an opaque struct. The platform header
            -- normally defines the real type before this fallback is needed.
            fallback_type =
                "typedef struct { uint64_t __pad[16]; } posix_spawn_file_actions_t;"
        end
        local ok = pcall(ffi.cdef, fallback_type)
        if not ok then
            return false
        end
    end
    ...
    if ffi.os == "OSX" then
        declarations[#declarations + 1] = "char ***_NSGetEnviron(void);"
    else
        declarations[#declarations + 1] = "extern char **environ;"
    end
    ...
    symbols[#symbols + 1] = ffi.os == "OSX" and "_NSGetEnviron" or "environ"
...
local function processEnvironment()
    local ok, environment = pcall(function()
        if ffi.os == "OSX" then
            local environment_pointer = C._NSGetEnviron()
            return environment_pointer ~= nil and environment_pointer[0] or nil
        end
        return C.environ
    end)
    if not ok or environment == nil then
        return nil,
            "Could not access the process environment required to launch the OCR worker"
    end
    return environment
end

local function newSpawnFileActions()
    if ffi.os == "OSX" then
        -- posix_spawn_file_actions_t is an opaque pointer on Darwin, so its
        -- init/destroy functions receive storage for that pointer.
        return ffi.new("posix_spawn_file_actions_t[1]")
    end
    return ffi.new("posix_spawn_file_actions_t")
end
...
function Worker.start(options, listener)
    ...
    local environment, environment_error = processEnvironment()
    ...
    local actions = newSpawnFileActions()
    ...
    repeat
        ...
        local can_retry_from_temporary_directory = ffi.os ~= "OSX"
            and spawn_result == posixConstant("EACCES", 13)
        if not ok or not can_retry_from_temporary_directory then
            break
        end
    ...
    if spawn_result ~= 0 then
        local macos_blocked = ffi.os == "OSX"
            and (
                spawn_result == posixConstant("EACCES", 13)
                or spawn_result == posixConstant("EPERM", 1)
            )
        if macos_blocked then
            return failAfterResolve(
                "macOS blocked the OCR worker (error " .. spawn_result .. "). "
                .. "Verify that the plugin came from the official release, "
                .. "then allow it in Privacy & Security or remove the "
                .. "com.apple.quarantine attribute from mangaocr.koplugin."
            )
        end
